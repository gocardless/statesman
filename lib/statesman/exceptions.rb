  ...
  class IncompatibleSerializationError < StandardError
    ...
    def _message(transition_class_name)
      "#{transition_class_name}#metadata column type cannot be json " \
        ...
        "column type, `include Statesman::Adapters::ActiveRecordTransitionAttributes` " \
        "instead of `Statesman::Adapters::ActiveRecordTransition`"
    end
  end

  class MissingTransitionAttributesError < StandardError
    def initialize(transition_class_name)
      super(_message(transition_class_name))
    end
    ...
    def _message(transition_class_name)
      "#{transition_class_name} does not define `.updated_timestamp_column`. " \
        "`include Statesman::Adapters::ActiveRecordTransition`, or " \
        "`include Statesman::Adapters::ActiveRecordTransitionAttributes` if you " \
        "are using a json column type"
    end
  end
end
