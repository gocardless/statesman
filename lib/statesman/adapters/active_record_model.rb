    ...
    module ActiveRecordModel
      ...
      module ClassMethods
        ...
        def state_machine_class
          raise NotImplementedError, "A state_machine_class method should be " \
                                     "defined on the model"
        end
        ...
        def state_inclusion_where(states)
          if initial_state.in?(states)
            'transition1.to_state IN (?) OR transition1.to_state IS NULL'
          else
            'transition1.to_state IN (?) AND transition1.to_state IS NOT NULL'
          end
        end

        def initial_state
          state_machine_class.initial_state.to_s
        end
      end
    end
  end
